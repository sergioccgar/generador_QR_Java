...
1. Abrir una terminal estando en el folder src
2. Ejecutar
```
javac *.java
```
...
### Puntos extras:
1. Se pueden generar QRs de cualquier tamaño.
2. Las imágenes se generan y se guardan en el mismo directorio.
3. Enlace de Youtube codificado. Es música de Zelda 
4. QR "ADIOSTEORIADECODIGOS" creado. El problema protagonista de esta práctica: no puede escanearse.
5. La imagen del punto extra 4 se llama "PuntoExtraCuatro.png" y la imagen creada con qrencode es "pe4.png"
La versión creada por qrencode es menor, es v1, no tiene patrón de alineamiento; la versión creada por mi
programa es versión dos. Yo creo que es porque qrencode usó la codificación alfanumérica y no la bytemode,
lo que permite almacenar más caracteres en un QR más pequeño. La principal diferencia es que el QR de qrencode
sí es escaneable, jeje :(.
